...
/* ---- Hosted PNG icons (Gmail/Outlook-safe) -------------------------- *
 *
 *  Gmail and Outlook STRIP inline data:image/svg+xml (SVG can carry script,
 *  so they block it for security) — every SVG icon renders as a broken block.
 *  The only reliable cross-client path is a hosted RASTER image referenced by
 *  absolute URL, which is what every real signature tool (WiseStamp, etc.) does.
 *
 *  These PNGs live in the Opsette Tools apex repo and are served at
 *  tools.opsette.io/signature-icons/. Generated by
 *  opsette-tools.github.io/_brand-icons/gen-signature-icons.mjs from the SAME
 *  glyph paths below. Contact icons are pre-rendered in each brand-preset accent
 *  (+ a neutral fallback for custom colors); social icons in their brand color.
 */
const HOSTED_ICON_BASE = "https://tools.opsette.io/signature-icons";

// Accent hexes we pre-rendered contact icons for. A signature whose accent is
// one of these gets an accent-tinted icon; anything else falls back to neutral.
const HOSTED_CONTACT_COLORS = new Set([
  "c2410c",
  "4338ca",
  "0f766e",
  "be123c",
...
/** Map a signature's accent hex to the color-key of its hosted contact icon. */
function hostedColorKey(accent: string): string {
  const key = (accent || "").replace(/^#/, "").toLowerCase();
  return HOSTED_CONTACT_COLORS.has(key) ? key : "neutral";
}

/** Hosted contact icon (envelope/phone/globe/pin) as a cross-client <img>. */
export function hostedContactIcon(
  key: ContactIconKey,
  accent: string,
  size: number,
): string {
  // "web"/"address" map to file keys web/address; email/phone are 1:1.
  const src = `${HOSTED_ICON_BASE}/contact-${key}-${hostedColorKey(accent)}.png`;
  return `<img src="${src}" alt="" width="${size}" height="${size}" style="display:block;width:${size}px;height:${size}px;border:0;" />`;
}

/** Hosted social icon as a cross-client <img>. `white` swaps to the white
 *  variant for icons sitting on a colored bar (Contact Rail). */
export function hostedSocialIcon(key: SocialKey, size: number, white = false): string {
  const src = `${HOSTED_ICON_BASE}/social-${key}${white ? "-white" : ""}.png`;
  return `<img src="${src}" alt="${key}" width="${size}" height="${size}" style="display:block;width:${size}px;height:${size}px;border:0;" />`;
}
 ...
 *
 *  NOTE: these SVG data-URI icons DON'T render in Gmail/Outlook — prefer the
 *  hosted PNG helpers above for the shipped templates. Kept for the app's own
 *  preview thumbnails where data URIs are fine.
 */
type SocialKey =
  | "linkedin"
  | "twitter"
...
export function socialIconsRow(
  ...
  return items
    .map((i) => {
      // "brand": the real brand mark. Ships the HOSTED PNG so it renders in
      // Gmail/Outlook (the inline SVG version is stripped by both).
      if (variant === "brand") {
        return `<a href="${normalizeUrl(i.url)}" style="display:inline-block;margin-right:${gap + 2}px;text-decoration:none;line-height:0;vertical-align:middle;">${hostedSocialIcon(i.key, size)}</a>`;
      }
      if (variant === "plain") {
        // Contact Rail passes white icons on its colored bar. Ship the hosted
        // white PNG when white, else the hosted brand mark — both Gmail-safe.
        const isWhite = color.toLowerCase() === "#ffffff" || color.toLowerCase() === "#fff";
        return `<a href="${normalizeUrl(i.url)}" style="display:inline-block;margin-right:${gap + 2}px;text-decoration:none;line-height:0;vertical-align:middle;">${hostedSocialIcon(i.key, size, isWhite)}</a>`;
      }
...
/** A single contact glyph (envelope / phone / globe / pin), tinted to `color`.
 *  Ships the HOSTED PNG (Gmail/Outlook-safe) — the inline SVG data-URI it used
 *  to emit is stripped by Gmail. `color` is snapped to the nearest hosted accent
 *  (or neutral). Exported so the v2 structural templates lay their own rows. */
export function contactIcon(key: ContactIconKey, color: string, size: number): string {
  return hostedContactIcon(key, color, size);
}
...
export const LIGHT_SURFACE: Surface = {
  // Dark-mode strategy: email clients (Gmail, Apple Mail, Outlook) auto-INVERT
  // dark text to light in a dark inbox — but only if the text is genuinely dark.
  // Medium grays get left alone and vanish on dark. So we use near-black ink and
  // a solid dark slate for contact text, set NO background on the signature body,
